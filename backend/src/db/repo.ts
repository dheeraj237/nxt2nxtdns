import { db, newId } from './client.js';
...
export const profilesRepo = {
  ...
  create(accountId: string, profileId: string, displayName: string | null): ProfileRow {
    ...
    db.prepare(
      'INSERT INTO profiles (id, account_id, profile_id, display_name, is_master, created_at) VALUES (?, ?, ?, ?, 0, ?)',
  ...
  setMaster(id: string): void {
    const tx = db.transaction(() => {
      db.prepare('UPDATE profiles SET is_master = 0 WHERE is_master = 1').run();
      db.prepare('UPDATE profiles SET is_master = 1 WHERE id = ?').run(id);
    });
    tx();
  },
  getMaster(): ProfileRow | undefined {
    return db.prepare('SELECT * FROM profiles WHERE is_master = 1').get() as ProfileRow | undefined;
  },
};

...
import type { Profile, SyncDiff, SyncResult } from '@/lib/apiClient';
...
export function DiffPreviewModal({ title, diffs, profiles, results, applying, onConfirm, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/40 p-4">
      <div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white p-4 shadow-xl">
        <h2 className="mb-3 text-lg font-semibold text-slate-900">{results ? 'Sync results' : title}</h2>

        {!results && totalOps === 0 && <p className="text-sm text-slate-500">All selected profiles already match.</p>}
